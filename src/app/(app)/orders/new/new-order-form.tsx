  ...
  return (
    <form action={formAction} className="space-y-6">
      <section className="card p-4">
        <h2 className="mb-3 font-bold text-stone-900">Who is this for?</h2>
      ...
      <section className="card p-4">
        <h2 className="mb-1 font-bold text-stone-900">What did they order?</h2>
        <p className="mb-3 text-sm text-stone-500">Set a quantity for each item they want.</p>

        {products.length === 0 ? (
          <p className="text-sm text-stone-500">
            No products yet —{" "}
            <Link href="/products" className="font-semibold text-rose-700 underline">
              add one first
            </Link>
            .
          </p>
                ...
                <li key={p.id} className="flex items-center gap-3 py-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      className="h-9 w-14 rounded-lg border border-stone-300 text-center text-sm tabular-nums"
                    />
                    <button
                      type="button"
        ...
        <div className="mt-3 flex items-center justify-between border-t border-stone-200 pt-3">
          ...
          <span className="text-xl font-bold tabular-nums text-stone-900">{formatMoney(total)}</span>
        </div>
      </section>

      <section className="card space-y-4 p-4">
        <h2 className="font-bold text-stone-900">Money & dates</h2>
        ...
        <label className="flex items-center gap-3 rounded-xl bg-stone-50 px-3 py-3">
          <input type="checkbox" name="delivered" className="h-5 w-5 rounded accent-rose-700" />
          <span className="text-sm font-medium text-stone-700">Already handed over</span>
        </label>
        ...
        <div className="flex items-center justify-between border-t border-stone-200 pt-3 text-sm">
          ...
          <span
            className={`text-lg font-bold tabular-nums ${
              balance > 0 ? "text-rose-700" : balance < 0 ? "text-violet-700" : "text-emerald-700"
            }`}
          >
